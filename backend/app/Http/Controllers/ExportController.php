...
class ExportController extends Controller
{
    ...
    public function exportXlsx(Request $request)
    {
        try {
            ...
            $data = $payload['data'] ?? [];
            $sheetRows = $this->normalizeRowsForSpreadsheet($data);
            $xlsxBinary = $this->buildXlsxBinary($sheetRows, 'Export');
    ...
    private function normalizeRowsForSpreadsheet(array $rows): array
    {
        if (empty($rows)) {
    ...
    private function buildXlsxBinary(array $rows, string $sheetName): string
    {
        ...
        $zip->addFromString('xl/workbook.xml', $this->getXlsxWorkbookXml($sheetName));
    ...
    private function getXlsxContentTypesXml(): string
    {
    ...
    private function getXlsxRootRelsXml(): string
    {
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
    ...
    private function getXlsxWorkbookXml(string $sheetName): string
    {
        $cleanSheetName = $this->sanitizeSheetName($sheetName);
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            . '<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">'
            . '<sheets>'
            . '<sheet name="' . $this->escapeXml($cleanSheetName) . '" sheetId="1" r:id="rId1"/>'
            . '</sheets>'
            . '</workbook>';
    }

    private function getXlsxWorkbookRelsXml(): string
    {
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            . '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'
    ...
    private function buildWorksheetXml(array $rows): string
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($cells as $columnIndex => $value) {
                $cellRef = $this->columnNumberToName($columnIndex + 1) . $rowIndex;
