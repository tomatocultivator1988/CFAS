...
$script:backendHost = "192.168.11.40"
...
function Start-LaravelBackend {
    ...
    try {
        ...
        # Check if already running
        $existingProcess = Get-Process -Name "php" -ErrorAction SilentlyContinue | 
            Where-Object { $_.CommandLine -like "*artisan serve*" }
        
        if ($existingProcess) {
            Write-Host "Laravel Backend is already running" -ForegroundColor Yellow
            return $true
        }
        ...
        if (Test-Path $artisanPath) {
            ...
            $processInfo.Arguments = "artisan serve --host=$($script:backendHost) --port=$($script:backendPort)"
            ...
            Start-Sleep -Seconds 3
            Write-Host "Laravel Backend started successfully" -ForegroundColor Green
            return $true
        } else {
...
$service1.Text = "[OK] Apache Web Server (Frontend)"
...
$service3.Text = "[OK] Laravel Backend API Server"
...
$startButton.Add_Click({
    ...
    try {
        ...
        $progressLabel.Text = "[OK] System Started!"
