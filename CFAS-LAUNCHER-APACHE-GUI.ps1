# ============================================================================
# CFAS Exam System Launcher - Apache Edition with Modern GUI
# Professional GUI launcher with Apache backend support
# ============================================================================
...
$script:logoPath = Join-Path $PSScriptRoot "cfas-logo.jpg"
$script:frontendUrl = "http://192.168.11.40/exam-frontend"
$script:backendApiUrl = "http://192.168.11.40/exam-backend/public/api/health"
$script:desktopPath = [Environment]::GetFolderPath("Desktop")
$script:shortcutPath = Join-Path $script:desktopPath "CFAS Exam System.lnk"
$script:launcherPath = Join-Path $PSScriptRoot "START-CFAS-FINAL.bat"
$script:iconPath = Join-Path $PSScriptRoot "cfas-icon.ico"
...
function Create-DesktopShortcut {
    try {
        ...
        $Shortcut.Description = "CFAS Review Center Examination System"
...
function Start-MySQLService {
    try {
        ...
        $mysqlConfig = Join-Path $script:xamppPath "mysql\bin\my.ini"
        
        if (Test-Path $mysqlPath) {
            Start-Process -FilePath $mysqlPath -ArgumentList "--defaults-file=`"$mysqlConfig`"" -WindowStyle Hidden
...
function Test-BackendAPI {
    try {
        ...
        $response = Invoke-WebRequest -Uri $script:backendApiUrl -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop
        ...
        Write-Host "Backend API check failed (this is normal if Apache just started)" -ForegroundColor Yellow
        return $true  # Continue anyway
    }
}
...
$service1.Text = "[OK] Apache Web Server"
...
$service2.Text = "[OK] MySQL Database Server"
...
$service3.Text = "[OK] Backend API (through Apache)"
...
$startButton.Add_Click({
    ...
    try {
        ...
        # Test Backend API
        $statusLabel.Text = "Verifying Backend API..."
        ...
        $progressLabel.Text = "[SUCCESS] System Started!"
        ...
        [System.Windows.Forms.MessageBox]::Show(
            "Error starting services: $_`n`nPlease start Apache and MySQL manually from XAMPP Control Panel.",
            "Error",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
